...
const readFields = async (store, language, templateFile) => {
  ...
  await readXlsxFile(templateFile, { sheet: sheetName }).then(data => {
    fields = data[1].reduce(function(result, field, index) {
      result[data[2][index]] = { loc: field };
      return result;
    }, {});
  });
...
const AddTemplateDialog = withFirebase(
  ({ firebase, classes, open, setOpen, categories, setSnackbarProps }) => {
